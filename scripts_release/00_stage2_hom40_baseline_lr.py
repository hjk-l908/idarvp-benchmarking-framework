...
def main():
    ...
    # CPU-friendly character k-mer baseline features.
    seqs = df["Sequence"].astype(str).tolist()
    # analyzer='char' yields character-level n-grams.
    ...
    def eval_split(name, idx):
        ...
        # Per-label AUPRC/AUROC; skip labels that are constant within the split.
        rows = []
        ...
        out = pd.DataFrame(
            rows,
            columns=["split", "label", "pos", "neg", "AUROC", "AUPRC", "F1@0.5"],
        )
        return out
