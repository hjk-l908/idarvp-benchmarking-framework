...
def metrics(y_true, prob, thr=0.5):
    ...
    return dict(AUROC=auroc, AUPRC=auprc, F1=f1, MCC=mcc, Precision=prec, Recall=rec, Specificity=spec,
                TN=tn, FP=fp, FN=fn, TP=tp)

def find_best_thr_on_valid(yv, pv):
    # 找使 MCC 最大的 threshold（只在 valid 上找）
    best = (-1.0, 0.5)
...
def main():
    args = parse_args()
    files = dict(train=args.train, valid=args.valid, test_easy=args.test_easy, test_hard=args.test_hard)
    ...
    # 評估各 split：0.5 與 best_thr（best_thr 是從 valid 找的）
    rows = []
    for split in ["valid","test_easy","test_hard"]:
        ...
        rows.append([split, "thr=0.5", len(y), int(y.sum()), best_thr, best_mcc, *m05.values()])
        rows.append([split, f"thr={best_thr:.2f}", len(y), int(y.sum()), best_thr, best_mcc, *mbt.values()])

    cols = ["split","setting","n","pos","best_thr_from_valid","best_valid_MCC",
            "AUROC","AUPRC","F1","MCC","Precision","Recall","Specificity","TN","FP","FN","TP"]
