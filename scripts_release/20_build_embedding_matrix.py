...
def load_vec(pt_path):
    ...
    # Supports tensor, ndarray, and common dict-based FAIR ESM outputs.
    if isinstance(obj, torch.Tensor):
        v = obj
    elif isinstance(obj, np.ndarray):
        v = torch.from_numpy(obj)
    elif isinstance(obj, dict):
        # Try common embedding keys first.
        for k in ["mean", "embedding", "repr", "vector"]:
            ...
            # Fall back to FAIR ESM representation dictionaries when needed.
            if "mean_representations" in obj:
                ...
                # Mean-pool sequence-level representations if the loaded object is 2D.
                if v.ndim == 2:
                    v = v.mean(dim=0)
            else:
                raise ValueError(
                    f"Unrecognized dict keys in {pt_path}: {list(obj.keys())[:20]}"
                )
    else:
        raise ValueError(
            f"Unrecognized pt object type: {type(obj)} from {pt_path}"
        )
...
def main():
    ...
    ap.add_argument(
        "--pt_dir",
        required=True,
        help="Directory containing *.pt files and an optional manifest.tsv",
    )
    ...
    ap.add_argument(
        "--manifest",
        default=None,
        help="Optional manifest path; defaults to <pt_dir>/manifest.tsv if present",
    )
    args = ap.parse_args()
    ...
    if os.path.exists(manifest):
        ...
        pt_col = "pt_path" if "pt_path" in mf.columns else (
            "path" if "path" in mf.columns else None
        )
        if pt_col is None:
            raise SystemExit(
                f"[ERROR] manifest has no pt_path/path col: {manifest} "
                f"cols={list(mf.columns)}"
            )
